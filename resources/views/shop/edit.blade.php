...
<div class="container text-center">
    <h1 class="display-1 mt-5">Shop Edit</h1>
    <div class="my-5">
    ...
    <form action="{{ route('shop.update' , ['shop' => $shop]) }}" method="post" enctype="multipart/form-data">
        ...
        <div class="input-group">
            <div class="col-sm-2 mt-5 mx-auto">
                ...
                <input class="form-control" name="name" id="name" type="text" value='{{ $shop->name }}'/>
            </div>
            <div class="col-sm-2 mt-5 mx-auto">
                <label for="images" class="form-label">Image:</label>
                <input class="form-control" name="images[]" type="file" id="images" multiple/>
                <img src="{{ url('storage/images/'.$shop->images()->first()?->name) }}" class="img-thumbnail" width="300" height="250" alt="image">
            </div>
            @foreach($tags as $tag)
                <div class="form-check form-check-inline">
                    <input class="form-check-input" name="tags[]" type="checkbox" id="tagCheckBox" value="{{ $tag->id }}"
                    @if($shop->tags()->get()->pluck('id')->contains($tag->id))
                        {{ 'checked' }}
                        @endif>
            ...
            @endforeach
            <div>
                <button type="submit" class="btn btn-primary mb-3">Update Shop</button>
            </div>
        </div>
    </form>
</div>
